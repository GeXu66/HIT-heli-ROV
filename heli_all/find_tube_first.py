...
def find_black(img_src, top=35, left=35, width=600, height=500):
    ...
    ret, black_img = cv2.threshold(gray_img, 40, 255, cv2.THRESH_BINARY)
...
def camera_control(src, count, DEBUG, DL):
    if DEBUG:
        ...
        if len(track_img[0]) == 0:
            ...
            cv2.waitKey(1)
            gray_img, black_img, black_cord = find_black(img_src, top=50, left=35, width=600, height=480)
            # print(black_cord)
            if len(black_cord[0]) > 5000:
                text3 = 'Find Black!'
                black_img, flag = ls_find(black_cord, black_img)
            else:
                text3 = 'No Black!'
                flag = False
            # black_img = cv2.cvtColor(black_img, cv2.COLOR_GRAY2BGR)
        ...
        if len(track_img[0]) == 0:
            ...
            if len(black_cord[0]) > 5000:
                ...
                # morph = dl_detect(img_src, count, DL, dl_flag, DEBUG)
                return center_error, pre_angle, True, 0
            else:
                text3 = 'No Black!'
                dl_flag = False
