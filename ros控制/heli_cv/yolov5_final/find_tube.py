...
def dl_detect(img_src, count, DL=False, dl_flag=False):
    if DL:
        # 先启动检测文件
        if count == 0:
    ...
    if DEBUG:
        try:
            ...
            if len(track_img[0]) == 0:
                ...
                cv2.waitKey(1)
                dl_detect(img_src, count, DL, dl_flag)
                return now_error, pre_angle
        except:
            return 0, -1.0
    else:
        img_src, track_img, img = deal_pic(src)
...
while cap.isOpened():
    ...
    if frame is not None:
        camera_control(frame, count, DEBUG=False, DL=False)
    else:
        exit(0)
